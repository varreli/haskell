...
move g i p = if valid g i then [chop size (xs ++ [p] ++ ys)]
               else []
...
-- and reconstructing another tree that has depth n 
...
minimax (Node g ts)
   ...
   | turn g == X = Node (g, maximum ps) ts'
                   where
                      ts' = [minimax child | child <- ts] --alt: ts' = map minimax ts    
                      ps  = [p | Node (_,p) _ <- ts'] 
...
howMany :: Tree a -> Int
howMany (Node _ ts) = 1 + sum (map howMany ts)
